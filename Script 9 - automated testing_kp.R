...
#gov_list_final_red <- readRDS("Outputs/gov_list_final_red_kp.rds")
ri_iati_activities <- readRDS(file = "Outputs/ri_iati_activities_kp.rds")

test_that("UK government funder names are as expected", {
  
  gov_funders_actual <- all_projects_tidied %>%
    select(Funder) %>% 
    unique() %>% 
    arrange(Funder)
  
  expect_equal(gov_funders_expected, gov_funders_actual$Funder)
...
test_that("fund names are as expected", {
  ...
  gov_funds_actual <- all_projects_tidied %>%
    select(Fund) %>% 
    unique() %>% 
    arrange(Fund)
